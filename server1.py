...
async def analyze(
    ...
    # Infer object name from target image using YOLO.
    target_label = "object"
    target_results = yolo_model(target_crop, conf=0.05, verbose=False)
    if target_results and target_results[0].boxes is not None and len(target_results[0].boxes) > 0:
        names = target_results[0].names if hasattr(target_results[0], "names") else getattr(yolo_model, "names", {})
        best_idx = int(np.argmax([float(b.conf[0]) if hasattr(b, "conf") and len(b.conf) else 0.0 for b in target_results[0].boxes]))
        best_box = target_results[0].boxes[best_idx]
        cls_idx = int(best_box.cls[0]) if hasattr(best_box, "cls") and len(best_box.cls) else -1
        if isinstance(names, dict):
            target_label = str(names.get(cls_idx, "object"))
        elif isinstance(names, list) and 0 <= cls_idx < len(names):
            target_label = str(names[cls_idx])
    ...
    for match_rank, idx in enumerate(matched_indices, start=1):
        ...
        text = str(target_label).strip() if str(target_label).strip() else "object"
